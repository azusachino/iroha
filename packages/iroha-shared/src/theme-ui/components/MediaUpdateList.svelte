<script lang="ts">
  ...
  function updateLabel(update: MediaChange): string {
    if (update.change_kind === "provider_activity") return "Activity recorded";
    if (update.progress_percent != null || update.position != null)
      return "Reading progress";
    if (update.status) return `Status · ${update.status.replaceAll("_", " ")}`;
