<script lang="ts">
	...
	import { formatDistance, formatDuration, formatPace, formatHr } from '$lib/format';
	...
	async function sweep<T>(fn: (cursor?: string) => Promise<Page<T>>): Promise<T[]> {
		const out: T[] = [];
	...
	const ringData = $derived<Ring[]>(
		hasRing && dRow
			? [
					{ label: 'Move', value: dRow.move_kcal, goal: dRow.move_goal_kcal, unit: 'kcal', color: 'var(--ring-move)' },
					{ label: 'Exercise', value: dRow.exercise_min, goal: dRow.exercise_goal_min, unit: 'min', color: 'var(--ring-exercise)' },
					{ label: 'Stand', value: dRow.stand_hours, goal: dRow.stand_goal_hours, unit: 'h', color: 'var(--ring-stand)' }
				]
			: []
	);
	...
	const isLatest = $derived(daysWithData.length > 0 && day === daysWithData[daysWithData.length - 1]);
	const dayHasData = $derived(dailyByDay.has(day) || sleepByDay.has(day) || actByDay.has(day));
	...
	onMount(async () => {
		try {
			...
			daysWithData = [...new Set([...dailyByDay.keys(), ...sleepByDay.keys(), ...actByDay.keys()])].sort();
			if (daysWithData.length) day = daysWithData[daysWithData.length - 1];
		} catch (e) {
...
<section class="cockpit">
	<div class="scrubber tile glow">
		<button class="nav" aria-label="Previous day" onclick={() => shift(-1)}>‹</button>
		<button
			class="scrub-center"
		...
		{#if pickerOpen}
			<button class="pk-backdrop" aria-label="Close picker" onclick={() => (pickerOpen = false)}></button>
		...
		<div class="bento">
			...
			<a class="card tile" href="/daily">
				...
				{#if vitals.length}
					<dl class="vitals">
						{#each vitals as m}
							<div><dt>{m.l}</dt><dd>{num(m.v, m.d)}<span class="u">{m.u}</span></dd></div>
						{/each}
					</dl>
				{:else}
			...
			<div class="card tile wide">
				<header><span class="ic">⚡</span> <a class="hdr-link" href="/activities">Activities</a></header>
				{#if acts.length}
					<ul class="acts">
						{#each acts as a}
							<li>
								<a class="act-row" href={`/activities/${a.id}`}>
									...
									<span class="a-metrics">
										{#if a.distance_m}{formatDistance(a.distance_m)} · {/if}{formatDuration(
