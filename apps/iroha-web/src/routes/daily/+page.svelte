<script lang="ts">
	...
	const ringData = $derived<Ring[]>(
		latestRingDay
			? [
					{ label: 'Move', value: latestRingDay.move_kcal, goal: latestRingDay.move_goal_kcal, unit: 'kcal', color: 'var(--ring-move)' },
					{ label: 'Exercise', value: latestRingDay.exercise_min, goal: latestRingDay.exercise_goal_min, unit: 'min', color: 'var(--ring-exercise)' },
					{ label: 'Stand', value: latestRingDay.stand_hours, goal: latestRingDay.stand_goal_hours, unit: 'h', color: 'var(--ring-stand)' }
				]
			: []
	);
	...
	{:else}
		<div class="hero tile glow">
			<div class="hero-head">
				...
				{#if latestRingDay}<span class="hero-date">{formatDateOnly(latestRingDay.day)}</span>{/if}
			</div>
		...
		<div class="controls">
			<div class="seg" role="tablist" aria-label="Aggregation granularity">
				{#each ['day', 'month', 'year'] as const as g}
					<button role="tab" aria-selected={gran === g} class:active={gran === g} onclick={() => (gran = g)}>
		...
		<div class="table-wrap tile">
			<table>
				<thead>
					<tr>
						<th class="l">{gran === 'day' ? 'Day' : gran === 'month' ? 'Month' : 'Year'}</th>
				...
				<tbody>
					{#each table as d}
						<tr>
							...
							<td>{fmt(d.stand, 0)}</td>
							<td>{d.moveClosedPct == null ? '—' : `${Math.round(d.moveClosedPct)}%`}</td>
							<td>{fmt(d.steps, 0)}</td>
