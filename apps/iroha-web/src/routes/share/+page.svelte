<script lang="ts">
	...
	import { formatDistance, formatDuration, formatDate, formatDateOnly, formatSport, formatPace, formatHr } from '$lib/format';
	...
	const selectedYearRoutes = $derived.by<RouteFeature[]>(() => {
		...
		return routes.features.filter((f: any) => f.properties && f.properties.year === selectedYear);
	});
	...
	const filteredRoutes = $derived.by<RouteFeature[]>(() => {
		...
		return selectedYearRoutes.filter((f) => f.properties.sport_type === sportFilter);
	});

	const cityGroups = $derived.by(() => {
		const groups: Record<string, { city: string; count: number; runCount: number; sports: Set<string> }> = {};
		...
		return Object.values(groups).sort((a, b) => b.runCount - a.runCount || b.count - a.count);
	});
	...
	const mappedRoutes = $derived.by<RouteFeature[]>(() => {
		...
		return filteredRoutes.filter((f) => (f.properties.city || 'Unknown') === cityFilter);
	});
	...
	$effect(() => {
		...
		const io = new IntersectionObserver(
			(entries) => {
				if (entries[0].isIntersecting && hasMore && !activitiesLoading && !activitiesLoadingMore) {
...
{:else if summary}
	...
	<div class="mb-8 grid grid-cols-1 gap-3 sm:grid-cols-3">
		<StatTile label="Total distance" value={formatDistance(selectedYearTotals?.distance_m ?? 0)} />
		<StatTile label="Activities" value={(selectedYearTotals?.activity_count ?? 0).toLocaleString()} />
		<StatTile label="Total time" value={formatDuration(selectedYearTotals?.duration_s ?? 0)} />
	</div>
		...
		<div class="mb-8 rounded-lg border border-border bg-surface p-4">
			...
			<div class="flex flex-col gap-2">
				{#each summary.by_sport as sport (sport.key)}
					...
					>
						<div class="w-48 shrink-0 flex items-center min-w-0"><SportBadge sport={sport.key} /></div>
...
{:else}
	<div class="grid grid-cols-1 gap-4 lg:grid-cols-3 mb-8">
		...
		<div class="rounded-lg border border-border bg-surface p-4 flex flex-col max-h-[400px]">
			<div class="mb-3 flex items-baseline justify-between gap-2">
				<h3 class="text-sm font-semibold text-text-muted">
				{sportFilter ? `${formatSport(sportFilter)} cities` : 'Cities visited'} in {selectedYear}
			</h3>
				{#if cityFilter}
					<button class="text-xs text-accent underline" onclick={() => (cityFilter = null)}>
						Show all
					</button>
				{/if}
			</div>
			{#if cityGroups.length === 0}
				<p class="text-xs text-text-muted">No route coordinates available for this year.</p>
			{:else}
				<div class="grid grid-cols-2 gap-2 overflow-y-auto pr-1">
					{#each cityGroups as group}
						{@const intensity = group.runCount / maxRunCount}
						{@const heatClass = intensity >= 0.8 ? 'bg-accent text-white border-accent shadow-lg shadow-accent/15' :
											 intensity >= 0.5 ? 'bg-accent/40 border-accent/40 text-text' :
											 intensity >= 0.25 ? 'bg-accent/20 border-accent/25 text-text' :
											 group.runCount > 0 ? 'bg-accent/10 border-accent/15 text-text' :
						...
						>
							<div class="min-w-0">
								<div class="font-bold text-sm truncate" title={group.city}>{group.city}</div>
								<div class="text-[9px] opacity-75 truncate mt-1">
									{Array.from(group.sports).map(s => formatSport(s)).join(', ')}
								</div>
							</div>
							<div class="mt-3 flex items-center justify-between">
								<span class="text-[10px] font-semibold uppercase tracking-wider opacity-90">
					...
					<tr class="border-b border-border last:border-0 odd:bg-surface even:bg-surface/60">
						...
						<td class="px-3 py-2 text-text">
							...
							{:else}
								...
								{/if}
								<span class="text-xs text-text-muted">({formatDuration(activity.duration_s ?? activity.moving_time_s)})</span>
							{/if}
						</td>
					</tr>
				{/each}
