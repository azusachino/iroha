...
describe("currentActivityStreak", () => {
  ...
  it("counts one calendar day once even with multiple activities", () => {
    expect(
      currentActivityStreak([localTimestamp(10), localTimestamp(10)], NOW),
    ).toBe(1);
  });
  ...
  it("counts consecutive days ending today", () => {
    expect(
      currentActivityStreak(
        [localTimestamp(8), localTimestamp(9), localTimestamp(10)],
        NOW,
      ),
    ).toBe(3);
  });
  ...
  it("stops at a missing day", () => {
    expect(
      currentActivityStreak(
        [localTimestamp(7), localTimestamp(8), localTimestamp(10)],
        NOW,
      ),
    ).toBe(1);
  });
