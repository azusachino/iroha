...
export function currentActivityStreak(startedAt: string[], now: Date = new Date()): number {
	...
	let streak = 0;
	for (let date = startOfDay(now); activeDays.has(dayKey(date)); date = new Date(date.getTime() - DAY_MS)) {
		streak++;
	}
	return streak;
}
