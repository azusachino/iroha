...
This audit covers every top-level Svelte route plus sampled activity, media, and sleep detail routes. The baseline was captured with headless Chromium against `https://iroha.h.azusachino.com` before
...
This is a historical browser/network audit against `https://iroha.h.azusachino.com` after the v0.4.1 cache deployment and before the 2026-08-15 request/read correction. Each canonical route was
...
```sh
make web-visual-check BASE=https://iroha.h.azusachino.com THEME=field-journal ROUTE=overview
