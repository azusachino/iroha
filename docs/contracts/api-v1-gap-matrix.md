...
Status: v0.4.1 released contract inventory

This matrix records the current contract evidence for the private `/api/v1` surface. The static public export is documented separately; the v0.4.1 tag records the current contract, but this pre-1.0
project does not promise compatibility across future releases or introduce an `/api/v2` policy.
...
| Release stability         | RELEASED        | v0.4.1 passes `make check`, `make validate`, the release-candidate gate, public-site checks/build, and local k3s/live smoke; the source tag records the local release boundary. |
...
3. Rehearse the real upload-to-worker workflow before each future production release; the v0.4.1 rehearsal is evidence for that release, not a permanent compatibility guarantee.
4. Keep the route and example inventory synchronized as the next release evolves.
