...
flowchart LR
    ...
    CADDY --> LIVE["iroha.azusachino.com (public, Cloudflare Tunnel)<br/>iroha-public.h.azusachino.com (tailnet)"]
```
...
- **Manual spot-check**: visiting `https://iroha.azusachino.com/` after a schedule fires. There is no dashboard for "did today's export look right" beyond reading the numbers; that's an acceptable
...
[iroha.azusachino.com](https://iroha.azusachino.com/) via the deployment's Cloudflare Tunnel, and reachable internally at `iroha-public.h.azusachino.com` for verification. The former GitHub Pages
...
3. Verify the export job reaches a successful run (`kubectl -n <namespace> logs deploy/iroha-job` or equivalent), then verify the builder job's own run, then confirm
   [the public site](https://iroha.azusachino.com/) shows a current `Data as of` timestamp.
