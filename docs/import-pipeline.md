...
Connector syncs persist the current page cursor after each successfully stored snapshot. A connector failure, including HTTP 429, preserves that cursor so the next attempt resumes at the failed page.
Providers expose a parsed `Retry-After` delay to the queue; when present, the job's next `run_after` honors that delay instead of applying the generic retry backoff.
