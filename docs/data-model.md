...
The repository's `scripts/db.py` wrapper calls the mise-provided SQLx CLI. The migration directory is the schema source of truth; application code does not auto-migrate the database.
...
UUIDs should be UUIDv7, generated by the application so IDs are time-sortable. Keep `created_at`, `started_at`, and other domain timestamps as explicit indexed columns; UUIDv7 ordering is useful for
locality and cursor pagination, not a replacement for timestamp queries.
